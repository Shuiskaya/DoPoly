  ...
  <body>
    ...
    <div class="container">
      ...
      <?php
        ...
        $result = $mysql->query("SELECT * FROM `request` WHERE `status` = 'На рассмотрении'");
        while ($request = $result->fetch_assoc()) { ?>
            <div class="col-md-5 h-100 p-4 bg-body-tertiary border rounded-3">
                <h6>Номер заявки: <?php echo $request['id']; ?>. Тип: <?php echo $request['type']; ?></h6>
                <h6>ФИО: <?php echo $request['name']; ?></h6>
                <?php if($request['comment'] != '') {?>
                  <h6>Комментарий: <?php echo $request['comment']; ?></h6>
                <?php } ?>
                <img width="500" src="data:image/jpeg;base64, <?php echo base64_encode($request['photo']); ?>" alt=""><br><br>
                <form action="validation-form/ans_req.php" method="post">
                  <input type="text" class="form-control" name="id" id="id" placeholder="Введите номер заявки"><br>
